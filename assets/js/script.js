$(() => {
  ...
  Object.entries(blocks).forEach(([block, types], index) => {
    navContent += `<div class="block-type ${
      index ? "mt-3" : ""
    }">${block}</div>`;
  ...
  $("#copy-btn").on("click", (e) => {
    ...
    setTimeout(() => {
      $("#copy-btn").html(
        '<i class="fas fa-copy"></i>&nbsp; <span>Copy source code</span>'
      );
    }, 1000);
  });
  ...
  $(".btn-device-sel").on("click", (e) => {
    ...
    $(`.btn-device-sel:not([data-device='${targetDevice}'])`).addClass(
      "text-muted"
    );
    $(`.btn-device-sel[data-device='${targetDevice}']`).removeClass(
      "text-muted"
    );
